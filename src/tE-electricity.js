  ...
  gens: [
    ...
    () => {
      ...
      return { q: same ? T`Two positive charges of ${Q(Qa, 'nC')} and ${Q(Qb, 'nC')} are ${Q(d * 100, 'cm')} apart. What is the electric field strength exactly halfway between them?` : T`A charge of $+${QT(Qa, 'nC')}$ and a charge of $-${QT(Qb, 'nC')}$ are ${Q(d * 100, 'cm')} apart. What is the electric field strength exactly halfway between them?`,
        ...
        s: same ? T`Each charge alone gives $E = \frac{kQ}{r^2}$ with $r = ${M(r)}\,\mathrm{m}$: ${Q(sig(Ea), 'N/C')} and ${Q(sig(Eb), 'N/C')}. Both point away from their charges, so in opposite directions at the midpoint: $|${M(sig(Ea))} - ${M(sig(Eb))}| = ${QT(E, 'N/C')}$.` : T`Each charge alone gives $E = \frac{kQ}{r^2}$ with $r = ${M(r)}\,\mathrm{m}$: ${Q(sig(Ea), 'N/C')} and ${Q(sig(Eb), 'N/C')}. At the midpoint both fields point from + towards −, so they add: ${Q(E, 'N/C')}.` };
    },
    () => pick([
