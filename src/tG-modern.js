...
${Fig(planeSvg({ W: 340, H: 220, x: [0, 1.05], y: [0, 6], step: [0.1, 1], tickX: 0.2, xl: 'v/c', yl: 'γ', fns: [{ f: v => 1 / Math.sqrt(1 - v * v), to: 0.986 }], pts: [[0.6, 1.25, 'γ = 1.25', 'end', false, 8, -6], [0.8, 5 / 3, 'γ ≈ 1.67', 'end', false, 8, -8]], segs: [[1, 0, 1, 6, 'mf-c2', true]], label: T`The Lorentz factor stays near 1 at everyday speeds and rises steeply as v approaches c` }), T`The Lorentz factor is almost exactly 1 until about $0.3c$, then shoots up towards infinity as $v \to c$.`)}
...
${FigW(lightClockSvg(), T`A light clock: to an observer who sees it moving, the light travels a longer diagonal path at the same speed $c$, so each tick takes longer — time dilation.`)}
...
${Fig(photoSvg(), T`One photon, one electron: the photon's energy $hf$ frees the electron (work function $\phi$) and the rest becomes its kinetic energy.`)}
...
${Fig(planeSvg({ W: 340, H: 210, x: [0, 12], y: [-3, 4], ticks: false, xl: 'f', yl: sub('E', 'k,max'), fns: [{ f: f => 0.6 * f - 3, from: 5, to: 11.8 }, { f: f => 0.6 * f - 3, from: 0, to: 5, cls: 'mf-c2', dash: true }], pts: [[5, 0, 'f₀', 'end', false, 6, 16], [0, -3, '−φ', 'start', false, 8, 4]], texts: [[9.6, 1.6, T`slope = h`, 'end', 'mf-small']], label: T`Maximum kinetic energy of photoelectrons against frequency: zero below the threshold frequency, then a straight line of slope h` }), T`$E_{k,\max} = hf - \phi$: no electrons below $f_0$; above it a straight line of slope $h$ whose extension meets the axis at $-\phi$.`)}
<h3>Matter waves</h3>
...
${Fig(goldFoilSvg(), T`Rutherford's gold foil: almost all alpha particles pass straight through; the rare large deflections reveal a tiny, dense, positive nucleus.`)}
...
${FigW(balmerSvg(), T`The visible (Balmer) lines of hydrogen, from electrons dropping to $n = 2$ (wavelengths from the NIST atomic spectra database).`)}
...
${FigW(penetrationSvg(), T`Penetrating power: alpha is stopped by paper, beta by a few mm of aluminium; gamma is only weakened, even by several cm of lead.`)}
...
${Fig(planeSvg({ W: 380, H: 230, x: [0, 250], y: [0, 10], step: [50, 2], xl: T`mass number A`, yl: T`MeV per nucleon`, extra: (X, Y, BE = [[2, 1.11, 'H-2'], [4, 7.07, 'He-4'], [7, 5.61, 'Li-7'], [12, 7.68, 'C-12'], [16, 7.98, 'O-16'], [56, 8.79, 'Fe-56'], [84, 8.72, 'Kr-84'], [138, 8.39, 'Ba-138'], [235, 7.59, 'U-235']]) => sPline(BE.map(([a, e]) => [X(a), Y(e)]), 'mf-c1', ' fill="none" stroke-width="2"') + BE.map(([a, e, n], i) => sC(X(a), Y(e), 3.5, 'mf-dot') + ([0, 1, 5, 8].includes(i) ? sT(X(a) + (i === 0 ? 8 : 0), Y(e) - (i === 0 ? -4 : 9), n, 'mf-small', i === 0 ? 'start' : 'middle') : '')).join(''), label: T`Binding energy per nucleon against mass number, peaking at iron-56` }), T`Binding energy per nucleon (approximate values): it peaks at iron-56. Fusing light nuclei or splitting heavy ones both move towards the peak and release energy.`)}
<h3>Fission</h3>
...
${FigW(fissionSvg(), T`A chain reaction: each fission releases 2–3 neutrons that can split more uranium nuclei. In a reactor, control rods absorb enough of them to keep it steady.`)}
<h3>Fusion</h3>
